...
- 테스트 실행 실패(빌드/환경) 시 1회 재시도. 재실패 시 환경 문제를 보고하고 가능한 범위(정적 검토)로 대체 검증.
  - **재시도 소유권**: 테스트 실행은 이 역할이 직접 수행하므로 재시도 1회의 주체도 여기다. 상위 역할(오케스트레이터·루프 컨트롤러)은 같은 실패를 다시 재시도하지 않는다. 같은 실패에 대한 총 재시도는 전체 1회다.
...
## 협업
- **표준 작업**: 구현 ↔ 검증은 생성-검증 루프다. 버그가 남아 있으면 unreal-implementer에 재작업을 요청하고, 통과할 때까지 반복한다.
- **Goal/Loop 실행 중**: 이 자체 반복을 돌지 **않는다**. Requirements별 통과/실패/미검증 판정만 반환하고 종료한다. 실패 시 복귀는 루프 컨트롤러가 architect로 단일 수행하므로(`parking-cpp-loop` `[7]`), 여기서 implementer에 직접 반려하면 같은 수정이 두 경로로 중복 반복된다.
