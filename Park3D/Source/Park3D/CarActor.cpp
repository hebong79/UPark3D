...
void ACarActor::ApplyPlateNumberSdf(const FCarPlateFrame& Frame)
{
	if (bPlateNumberSdfBuilt || Frame.WidthCm <= 0.0 || Frame.HeightCm <= 0.0)
	{
		return;
	}
	...
	//
	// **슬롯 번호를 박으면 안 된다.** SM_Plate_F/B 는 슬롯 0 이 M_PlateFrame(테두리)이고
	...
	auto Bind = [this](UStaticMeshComponent* Plate, TObjectPtr<UMaterialInstanceDynamic>& OutMid)
	{
		...
		if (OutMid == nullptr)
		{
			const int32 SlotCount = Plate->GetNumMaterials();
			for (int32 Slot = 0; Slot < SlotCount; ++Slot)
			{
				UMaterialInterface* const Mat = Plate->GetMaterial(Slot);
		...
		OutMid->SetTextureParameterValue(TEXT("NumberSDF"), PlateNumberSdf);
		return true;
	};

	const bool bFront = Bind(FrontPlateComp, FrontPlateMid);
	const bool bBack = Bind(BackPlateComp, BackPlateMid);

	// 성공한 판만 폴백 위젯을 끈다. 아틀라스 에셋이나 메트릭 JSON 이 없으면 여기서 실패하는데,
	// 그때 위젯까지 꺼 버리면 번호가 통째로 사라진다.
	if (FrontPlateWidget) FrontPlateWidget->SetVisibility(!bFront, true);
	if (BackPlateWidget) BackPlateWidget->SetVisibility(!bBack, true);

	UE_LOG(LogTemp, Display,
		TEXT("[CarPlate] SDF id=%s number=%s 판=%.2fx%.2fcm rt=%s front=%d back=%d 폴백위젯=%d/%d"),
		*CarData.id, *PlateNumber, Frame.WidthCm, Frame.HeightCm,
		PlateNumberSdf ? TEXT("있음") : TEXT("없음"), bFront, bBack,
...
void ACarActor::UpdatePlatePresentation()
{
	...
	// 위 가시성 루프가 판을 켤 때 자식인 위젯도 같이 켜진다. SDF 가 이미 붙어 있다면(재초기화)
	// 여기서 다시 접어야 번호가 겹쳐 보이지 않는다.
	if (PlateNumberSdf != nullptr)
	{
		if (FrontPlateWidget && FrontPlateMid) FrontPlateWidget->SetVisibility(false, true);
		if (BackPlateWidget && BackPlateMid) BackPlateWidget->SetVisibility(false, true);
	}
}
