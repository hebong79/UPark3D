...
public class CCarObjListUI : MonoBehaviour
{
    ...
    public void Initialize(Camera kCamera, bool bRandomCreate = true)
    {
        if( kCamera == null)
        {
            kCamera = Camera.main;
        ...
        Reset_CreateCarObjectList(kCamera, bRandomCreate);
    }
    ...
    /// <summary>
    /// 차량 리스트에서 무작위로 count대를 선택해 표시 상태(SetActive)를 반전(토글)한다.
    /// count &lt;= 0 이면 전체 차량을 토글한다. count가 차량 수보다 크면 전체로 클램프.
    /// </summary>
    /// <returns>토글된 차량 리스트</returns>
    public List<CObjCar> ToggleRandomCars(int count)
    {
        CLogger.Inst.Debug($"========== ToggleRandomCars 시작: count={count} ==========", "CarList");

        List<CObjCar> toggledCars = new List<CObjCar>();
        ...
        if (validCars.Count == 0)
        {
            CLogger.Inst.Warning("토글할 차량이 없습니다 (유효 차량 없음)", "CarList");
            return toggledCars;
        }

        // count <= 0 이면 전체, 이후 클램프
        if (count <= 0) count = validCars.Count;
        count = Mathf.Min(count, validCars.Count);

        // 랜덤 시드 초기화
        int seed = (int)(DateTime.Now.Ticks ^ Environment.TickCount);
        UnityEngine.Random.InitState(seed);

        // 중복 없는 랜덤 인덱스 count개 선택
        HashSet<int> selectedIndices = new HashSet<int>();
        while (selectedIndices.Count < count)
        {
            selectedIndices.Add(UnityEngine.Random.Range(0, validCars.Count));
        }

        // 선택 차량 토글
        foreach (int idx in selectedIndices)
        {
            CObjCar car = validCars[idx];
            car.gameObject.SetActive(!car.gameObject.activeSelf);
            toggledCars.Add(car);
            CLogger.Inst.Debug($"차량 토글: {car.m_NameId} (idx={car.m_Idx}, active={car.gameObject.activeSelf})", "CarList");
        }

        CLogger.Inst.Debug($"========== ToggleRandomCars 완료: {toggledCars.Count}대 토글 ==========", "CarList");
        return toggledCars;
    }
    ...
    public void Reset_CreateCarObjectList(Camera kCamera, bool bRandomCreate=true)
    {
        ...
        for (int i = 0; i < kSaveData.datas.Count; i++)
        {
            ...
            Vector3 spawnPos = SnapToGround(kPos.pos.Get());
            if (bRandomCreate)
                kObjCar = CreateRandomCarObjectByCarPos(spawnPos, kPos, kCamera, ref carIdx);
            else
                kObjCar = CreateCarObjectByCarPos(spawnPos, kPos, kCamera);
    ...
    /// <summary>pos 하방 레이캐스트로 지면 Y를 구해 스폰 위치를 보정한다. 지면 미검출 시 원본 pos 반환.</summary>
    private Vector3 SnapToGround(Vector3 pos)
    {
        Vector3 origin = new Vector3(pos.x, pos.y + 10f, pos.z);
        if (Physics.Raycast(origin, Vector3.down, out RaycastHit hit, 20f, Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore))
            return new Vector3(pos.x, hit.point.y, pos.z);
        Debug.LogWarning($"[CCarObjListUI] 지면 미검출 (x={pos.x:F3}, z={pos.z:F3}) — 원본 y={pos.y:F3} 사용");
        return pos;
    }
