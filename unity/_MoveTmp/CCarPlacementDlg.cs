...
public class CCarPlacementDlg : CDialogUI, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public enum ERandomResetMode
    {
        ColorOnly = 0,
        ObjectAndColor = 1,
        CountObjectAndColor = 2,
    }
    ...
    [SerializeField] Button m_btnResetRandom;   // 랜덤으로 다시 차량배치
    [SerializeField] Button m_btnExit;      // 종료
    [SerializeField] Dropdown m_cboResetRandomType; // ResetInfo/cboResetRandomType
    ...
    /// <summary>배치 중에만 충돌을 무시하도록 설정한 차량 Collider 쌍.</summary>
    struct SColliderPair
    {
        public Collider m_First;
        public Collider m_Second;
    }

    List<SColliderPair> m_IgnoredVehicleCollisionPairs = new List<SColliderPair>();
    ...
    void Start()
    {
        ...
        if (m_btnResetRandom != null)
            m_btnResetRandom.onClick.AddListener(OnClicked_ResetRandom);
    ...
    public void Initialize()
    {
        ...
        // 차량 대화상자 진입 시 저장된 차량 프리팹·위치·색상을 그대로 유지한다.
        m_CarObjListUI.Initialize(m_CurCamera, false);
        ApplyVehiclePhysicsState(CDataMgr.Inst.IsCarPlacementState());
    ...
    void Callback_DealyStart()
    {
        ...
        // 차량 대화상자 자동 초기화에서도 저장된 배치를 랜덤으로 덮어쓰지 않는다.
        m_CarObjListUI.Initialize(m_CurCamera, false);
        ApplyVehiclePhysicsState(CDataMgr.Inst.IsCarPlacementState());
        //m_CurCarPrefab = m_CarObjListUI.GetCarPrefab(0);
        m_KeyType = EKeyType.Move;
    ...
    /// <summary>
    /// 초기화 버튼과 car.clear RPC가 공유하는 전체 삭제 동작.
    /// </summary>
    public int ClearAllCarPlacement()
    {
        ClearMultiSelection();
        m_fRotateY = 0;

        if (m_CarObjListUI == null)
        {
            CDataMgr.Inst.RemoveAll_CarPosList();
            RemoveAll_ScrollItems();
            return 0;
        }

        int removedCount = m_CarObjListUI.m_ObjCarList != null
            ? m_CarObjListUI.m_ObjCarList.Count
            : 0;
        CDataMgr.Inst.m_SaveCarPosData.Clear();
        m_CarObjListUI.RemoveAll();
        m_CarObjListUI.m_CurCarIDList.Clear();
        m_CarObjListUI.m_SelectdCarObject = null;
        m_SelectdCarObject = null;
        RemoveAll_ScrollItems();

        Debug.Log($"[CCarPlacementDlg] 차량 전체 초기화 완료: {removedCount}대 삭제");
        return removedCount;
    }

    void OnClicked_ResetRandom()
    {
        ERandomResetMode mode = m_cboResetRandomType != null
            ? (ERandomResetMode)m_cboResetRandomType.value
            : ERandomResetMode.ObjectAndColor;
        int requestedCount = ReadRequestedRandomCarCount();
        ResetRandomPlacement(mode, requestedCount);
    }

    /// <summary>
    /// UI와 RPC가 공유하는 랜덤 리셋 진입점.
    /// </summary>
    /// <param name="mode">색상만, 오브젝트+색상, 차량 수+오브젝트+색상</param>
    /// <param name="requestedCount">모드 3의 차량 수. 0 이하는 슬롯 범위에서 랜덤 선택.</param>
    /// <returns>처리된 차량 수</returns>
    public int ResetRandomPlacement(ERandomResetMode mode, int requestedCount = 0)
    {
        if (m_CarObjListUI == null)
        {
            Debug.LogWarning("[CCarPlacementDlg] 차량 목록 UI가 없어 랜덤 리셋을 수행할 수 없습니다.");
            return 0;
        }
        ...
        {
            int coloredCount = 0;
            foreach (CObjCar car in m_CarObjListUI.m_ObjCarList)
            {
                if (car == null || car.gameObject == null || !car.gameObject.activeSelf)
                    continue;

                car.SetVehicleColor(CSaveCarPosData.GetRandomColor());
                coloredCount++;
            }

            Debug.Log($"[CCarPlacementDlg] 랜덤 리셋 완료: 색상만 {coloredCount}대");
            return coloredCount;
        }

        List<CPresetSlotPlacer.SSlotRef> loadedSlots = CollectLoadedParkingSlots();
        if (loadedSlots.Count == 0)
        {
            ClearAllCarPlacement();
            Debug.LogWarning("[CCarPlacementDlg] 로드된 주차면이 없어 랜덤 리셋 차량 0대입니다.");
            return 0;
        }
        ...
        if (mode == ERandomResetMode.CountObjectAndColor)
        {
            targetCount = requestedCount > 0
                ...
                : UnityEngine.Random.Range(1, loadedSlots.Count + 1);
        ...
        Camera placementCamera = m_CurCamera != null ? m_CurCamera : Camera.main;
        Dictionary<int, List<CFaceRect>> slotsByPreset = new Dictionary<int, List<CFaceRect>>();
        foreach (CPresetSlotPlacer.SSlotRef slotRef in loadedSlots)
        {
            if (!slotsByPreset.TryGetValue(slotRef.presetId, out List<CFaceRect> presetSlots))
            {
                presetSlots = new List<CFaceRect>();
                slotsByPreset.Add(slotRef.presetId, presetSlots);
            }
            presetSlots.Add(slotRef.slot);
        }

        int placedCount = 0;
        foreach (KeyValuePair<int, List<CFaceRect>> presetSlots in slotsByPreset)
        {
            List<CObjCar> placedCars = CPresetSlotPlacer.PlaceVehiclesOnSlots(
                m_CarObjListUI,
                presetSlots.Value,
                presetSlots.Key,
                placementCamera,
                ...
                true,
                true);
            placedCount += placedCars.Count;
        }

        RefreshCarPlacementListSelection();
        Debug.Log($"[CCarPlacementDlg] 랜덤 리셋 완료: 모드={mode}, 목표 {targetCount}대 / 배치 {placedCount}대");
        return placedCount;
    }

    int ReadRequestedRandomCarCount()
    {
        if (m_InCreateCarNum == null || string.IsNullOrWhiteSpace(m_InCreateCarNum.text))
            return 0;

        return int.TryParse(m_InCreateCarNum.text, out int count) ? count : 0;
    }

    void RefreshCarPlacementListSelection()
    {
        Reset_CreateScrollViewCarItem();
        if (m_CarScrollView == null || m_CarScrollView.content == null || m_CarScrollView.content.childCount == 0)
            return;

        CItemText firstItem = m_CarScrollView.content.GetChild(0).GetComponent<CItemText>();
        if (firstItem != null)
            OnSelected_CarObjItem(firstItem, 0, true);
    }

    List<CPresetSlotPlacer.SSlotRef> CollectLoadedParkingSlots()
    {
        List<CPresetSlotPlacer.SSlotRef> loadedSlots = new List<CPresetSlotPlacer.SSlotRef>();
        CPMakerParkSpaceUI parkSpaceUI = FindObjectOfType<CPMakerParkSpaceUI>();
        if (parkSpaceUI == null || parkSpaceUI.m_PresetObjList == null)
            return loadedSlots;

        foreach (CPMakerParkSpaceUI.SPresetObjUI presetObj in parkSpaceUI.m_PresetObjList)
        {
            if (presetObj == null || presetObj.m_listFaceRect == null)
                continue;

            foreach (CFaceRect slot in presetObj.m_listFaceRect)
            {
                if (slot != null)
                    loadedSlots.Add(new CPresetSlotPlacer.SSlotRef(presetObj.m_PresetIdx, slot));
            }
        }
    ...
    void ShuffleSlots(List<CPresetSlotPlacer.SSlotRef> slots)
    {
        for (int i = slots.Count - 1; i > 0; i--)
        {
            int swapIndex = UnityEngine.Random.Range(0, i + 1);
            CPresetSlotPlacer.SSlotRef temp = slots[i];
    ...
    /// <summary>현재 배치 플래그를 모든 차량의 Rigidbody에 적용한다.</summary>
    void ApplyVehiclePhysicsState(bool bPlacement)
    {
        if (m_CarObjListUI == null || m_CarObjListUI.m_ObjCarList == null)
            return;

        foreach (CObjCar car in m_CarObjListUI.m_ObjCarList)
        {
            if (car != null)
                car.SetPlacementPhysics(bPlacement);
        }
    ...
    /// <summary>서로 다른 차량 Collider 쌍만 배치 중 충돌에서 제외한다.</summary>
    void IgnoreVehicleCollisions()
    {
        List<CObjCar> cars = m_CarObjListUI.m_ObjCarList;
        for (int i = 0; i < cars.Count; i++)
        {
            CObjCar firstCar = cars[i];
            if (firstCar == null) continue;

            Collider[] firstColliders = firstCar.GetComponentsInChildren<Collider>(true);
            for (int j = i + 1; j < cars.Count; j++)
            {
                CObjCar secondCar = cars[j];
                if (secondCar == null) continue;

                Collider[] secondColliders = secondCar.GetComponentsInChildren<Collider>(true);
                foreach (Collider first in firstColliders)
                {
                    if (first == null) continue;
                    foreach (Collider second in secondColliders)
                    {
                        if (second == null || Physics.GetIgnoreCollision(first, second)) continue;

                        Physics.IgnoreCollision(first, second, true);
                        m_IgnoredVehicleCollisionPairs.Add(new SColliderPair
                        {
                            m_First = first,
                            m_Second = second,
                        });
    ...
    /// <summary>배치 중에 제외한 차량 Collider 쌍의 충돌을 복원한다.</summary>
    void RestoreVehicleCollisions()
    {
        foreach (SColliderPair pair in m_IgnoredVehicleCollisionPairs)
        {
            if (pair.m_First != null && pair.m_Second != null)
                Physics.IgnoreCollision(pair.m_First, pair.m_Second, false);
        }
        m_IgnoredVehicleCollisionPairs.Clear();
    }
    ...
    {
        ApplyVehiclePhysicsState(false);
        Text kText = m_btnPlacementStart.GetComponentInChildren<Text>();
        kText.text = "배치 시작";
    ...
    {
        if (m_CarScrollView == null || m_CarScrollView.content == null)
            return;

        // Destroy는 프레임 끝까지 자식으로 남을 수 있으므로 먼저 분리한다.
        // Reset 직후 새 항목을 만들 때 이전 템플릿/항목이 한 줄 남아 보이는 것을 방지한다.
    ...
    public void CheckPickingPositionByMouse()
    {
        ...
        else
        {
            if (Input.GetMouseButtonDown(0)) // 마우스 왼쪽 버튼 클릭
            {
                ...
                if (Physics.Raycast(ray, out hit, Mathf.Infinity, kLayerMask, QueryTriggerInteraction.Collide))
                {
