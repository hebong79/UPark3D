...
- 정본 `Package/Windows`·`Work13520` 은 옛 빌드다 — exe 만 갈면 MI 를 못 찾아 **옛 판(흰 필름식)으로 폴백**한다(로그 `[PlateSDF] 종류 머티리얼 없음`). 새 판은 아래 4.1/4.2 의 `Work13540` 처럼 **재쿡한 패키지**에서만 나온다. 정본 교체는 사용자 결정.

## 4.1 패키지 빌드에서만 드러난 것 — 유니티 빌드 충돌 (20:40 추가)

`BuildPackage.bat Work13540` 첫 시도가 컴파일에서 죽었다. 원인 둘, 전부 **익명 네임스페이스 도우미의 중복 정의(C2084)**:
`PlateKinds.cpp`/`PlateLayout.cpp` 의 `IsDigitCh`, 그리고 `CarPlateRpcExtTest.cpp`/`EnvSceneRpcModuleTest.cpp` 의 `StrField`/`NumField`.
`Build.bat`(적응형 유니티 — 바뀐 파일은 유니티에서 빠진다)로는 통과하고 UAT 의 전체 유니티에서만 한 TU 로 묶여 터진다
(CLAUDE.md 09-07 `ApplyWhiteDropdown` 항목과 같은 함정). 글자 판정은 `PlateKinds.h` 인라인으로 올리고, 테스트 도우미는 `Cp` 접두사를 붙였다
(브랜치 `fix/unity-build-collisions`). **새 .cpp 를 추가했으면 커밋 전에 패키지 빌드(전체 유니티)를 한 번 돌릴 것.**

## 4.2 패키지 검증 (20:45 추가)

`Package/Work13540/Windows`(RPC 13540 · main 13960 · cam 13961~13999 — 정본 13510 과 동시 실행) — 쿡 2351 패키지 3 분(DDC 온).
매니페스트에 `MI_Plate_*` 10 · `T_Plate_*` 30 · `M_PlateKind` 확인. 기동 로그에 `종류 머티리얼 없음`·`Failed to compile Material` 없음,
23대가 7종으로 배정, 근접 캡처(`_workspace/plate_shots/pkg13540_sheet.jpg`)에서 페인트식·전기차·필름식 판이 `-game` 과 같게 나왔다.
**패키지의 기본 config 는 정본과 포트가 같으므로** 두 번째 인스턴스는 `Save/Config/config_pmaker.json` 의 `rpc_port`/`main_port`/`cam_port_*` 를 바꿔 띄운다
(`-RpcPort=` 는 RPC 만 덮고 스트림 포트는 못 덮는다).
